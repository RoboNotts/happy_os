    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ModbusResponseError::CommandParseError(e)
                => write!(f, "Failed to parse response. Reason: {}", e),
            ModbusResponseError::CheckSumFail
                => write!(f, "Failed to validate response checksum"),
        }
    }
}
...
impl ModbusResponse {
    pub fn from_reader(buf : &mut dyn Read) -> Result<ModbusResponse, ModbusResponseError> {
        ...
        let command: ModbusCommand = message_start[1].try_into()
        ...
        match command {
            ModbusCommand::WriteRegister => {
                ...
                let register: ModbusRegister = (((message_end[0] as u16) << 8) + (message_end[1] as u16)).try_into()
