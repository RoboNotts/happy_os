...
impl Display for MotorControllerError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::SerialError(e) => {
                write!(f, "Error initialising serial connection. Reason: {}", e)
            }
            ...
            Self::IncorrectDataLength(expected, got) => write!(
                f,
                "Invalid response length. Expected {}, got {}",
                expected, got
            ),
            Self::IncorrectResponseType => write!(f, "Incorrect response type"),
            Self::MotorStatusParseError(e) => {
                write!(f, "Failed to parse motor status. Reason: {}", e)
            }
        }
    }
}
